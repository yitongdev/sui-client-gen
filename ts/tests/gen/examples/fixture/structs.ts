...
import { Balance } from "../../sui/balance/structs.js";
import { ID, UID } from "../../sui/object/structs.js";
import { SUI } from "../../sui/sui/structs.js";
import { Url } from "../../sui/url/structs.js";
...
export interface FooFields<T extends TypeArgument> {
  ...
  genericVecNested: ToField<Vector<WithTwoGenerics<T, "u8">>>;
  ...
  twoGenericsReifiedNested: ToField<
    WithTwoGenerics<Bar, WithTwoGenerics<"u8", "u8">>
  >;
...
export class Foo<T extends TypeArgument> implements StructClass {
  ...
  readonly genericVecNested: ToField<Vector<WithTwoGenerics<T, "u8">>>;
  ...
  readonly twoGenericsNested: ToField<
    WithTwoGenerics<T, WithTwoGenerics<"u8", "u8">>
  >;
  ...
  static get bcs() {
    return <T extends BcsType<any>>(T: T) =>
      bcs.struct(`Foo<${T.name}>`, {
        ...
        two_generics_reified_primitive: WithTwoGenerics.bcs(
          bcs.u16(),
          bcs.u64(),
        ),
        two_generics_reified_object: WithTwoGenerics.bcs(Bar.bcs, Bar.bcs),
        two_generics_nested: WithTwoGenerics.bcs(
          T,
        ...
        two_generics_nested_vec: bcs.vector(
          WithTwoGenerics.bcs(
            Bar.bcs,
            bcs.vector(WithTwoGenerics.bcs(T, bcs.u8())),
          ),
        ),
        dummy: Dummy.bcs,
...
> implements StructClass
{
  ...
  >(
    ...
    return {
      ...
      reifiedTypeArgs: [T, U],
      fromFields: (fields: Record<string, any>) =>
        WithSpecialTypes.fromFields([T, U], fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
      ...
      fromJSONField: (field: any) =>
        WithSpecialTypes.fromJSONField([T, U], field),
      fromJSON: (json: Record<string, any>) =>
        WithSpecialTypes.fromJSON([T, U], json),
      fromSuiParsedData: (content: SuiParsedData) =>
  ...
  >(
    ...
    ToTypeStr<WithSpecialTypes<ToPhantomTypeArgument<T>, ToTypeArgument<U>>>
  > {
...
export type WithSpecialTypesInVectorsReified<T extends TypeArgument> = Reified<
