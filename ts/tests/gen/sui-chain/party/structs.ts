...
export function isPermissions(type: string): boolean {
  type = compressSuiType(type);
  return type === `${PKG_V31}::party::Permissions`;
}
...
export class Permissions implements StructClass {
  ...
  static readonly $typeName = `${PKG_V31}::party::Permissions`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = Permissions.$typeName;
  readonly $fullTypeName: `${typeof PKG_V31}::party::Permissions`;
  readonly $typeArgs: [];
  ...
  private constructor(typeArgs: [], fields: PermissionsFields) {
    this.$fullTypeName = composeSuiType(
      ...
      ...typeArgs,
    ) as `${typeof PKG_V31}::party::Permissions`;
    this.$typeArgs = typeArgs;
