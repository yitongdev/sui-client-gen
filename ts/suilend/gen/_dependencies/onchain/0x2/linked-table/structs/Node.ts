...
export function isNode(type: string): boolean {
  ...
  return type.startsWith(`${PKG_V35}::linked_table::Node` + "<");
}
...
> = Reified<Node<T0, T1>, NodeFields<T0, T1>>;
  ...
  implements StructClass
{
  ...
  static readonly $typeName = `${PKG_V35}::linked_table::Node`;
  static readonly $numTypeParams = 2;
  static readonly $isPhantom = [false, false] as const;

  readonly $typeName = Node.$typeName;
  readonly $fullTypeName: `${typeof PKG_V35}::linked_table::Node<${ToTypeStr<T0>}, ${ToTypeStr<T1>}>`;
  ...
  private constructor(
    ...
    fields: NodeFields<T0, T1>,
  ) {
    this.$fullTypeName = composeSuiType(
      Node.$typeName,
      ...typeArgs,
    ) as `${typeof PKG_V35}::linked_table::Node<${ToTypeStr<T0>}, ${ToTypeStr<T1>}>`;
  ...
  >(T0: T0, T1: T1): NodeReified<ToTypeArgument<T0>, ToTypeArgument<T1>> {
    return {
      typeName: Node.$typeName,
      fullTypeName: composeSuiType(
        Node.$typeName,
        ...[extractType(T0), extractType(T1)],
      ) as `${typeof PKG_V35}::linked_table::Node<${ToTypeStr<ToTypeArgument<T0>>}, ${ToTypeStr<ToTypeArgument<T1>>}>`,
      ...
      fromFields: (fields: Record<string, any>) =>
        Node.fromFields([T0, T1], fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        Node.fromFieldsWithTypes([T0, T1], item),
      ...
      fromSuiParsedData: (content: SuiParsedData) =>
        Node.fromSuiParsedData([T0, T1], content),
      fromSuiObjectData: (content: SuiObjectData) =>
        Node.fromSuiObjectData([T0, T1], content),
      fetch: async (client: SuiClient, id: string) =>
        Node.fetch(client, [T0, T1], id),
