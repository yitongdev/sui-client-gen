...
export function isJWK(type: string): boolean {
  ...
  return type === `${PKG_V35}::authenticator_state::JWK`;
}
...
export class JWK implements StructClass {
  ...
  static readonly $typeName = `${PKG_V35}::authenticator_state::JWK`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = JWK.$typeName;
  readonly $fullTypeName: `${typeof PKG_V35}::authenticator_state::JWK`;
  readonly $typeArgs: [];
  ...
  private constructor(typeArgs: [], fields: JWKFields) {
    this.$fullTypeName = composeSuiType(
      JWK.$typeName,
      ...typeArgs,
    ) as `${typeof PKG_V35}::authenticator_state::JWK`;
