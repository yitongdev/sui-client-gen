...
export function isJwkId(type: string): boolean {
  ...
  return type === `${PKG_V35}::authenticator_state::JwkId`;
}
...
export class JwkId implements StructClass {
  ...
  static readonly $typeName = `${PKG_V35}::authenticator_state::JwkId`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = JwkId.$typeName;
  readonly $fullTypeName: `${typeof PKG_V35}::authenticator_state::JwkId`;
  readonly $typeArgs: [];
  ...
  private constructor(typeArgs: [], fields: JwkIdFields) {
    this.$fullTypeName = composeSuiType(
      JwkId.$typeName,
      ...typeArgs,
    ) as `${typeof PKG_V35}::authenticator_state::JwkId`;
