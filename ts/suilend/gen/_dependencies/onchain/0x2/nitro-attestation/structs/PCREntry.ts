...
export function isPCREntry(type: string): boolean {
  ...
  return type === `${PKG_V35}::nitro_attestation::PCREntry`;
}
...
export class PCREntry implements StructClass {
  ...
  static readonly $typeName = `${PKG_V35}::nitro_attestation::PCREntry`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = PCREntry.$typeName;
  readonly $fullTypeName: `${typeof PKG_V35}::nitro_attestation::PCREntry`;
  readonly $typeArgs: [];
  ...
  private constructor(typeArgs: [], fields: PCREntryFields) {
    this.$fullTypeName = composeSuiType(
      PCREntry.$typeName,
      ...typeArgs,
    ) as `${typeof PKG_V35}::nitro_attestation::PCREntry`;
