...
export function isSetting(type: string): boolean {
  ...
  return type.startsWith(`${PKG_V35}::config::Setting` + "<");
}
...
export class Setting<T0 extends TypeArgument> implements StructClass {
  ...
  static readonly $typeName = `${PKG_V35}::config::Setting`;
  static readonly $numTypeParams = 1;
  static readonly $isPhantom = [false] as const;

  readonly $typeName = Setting.$typeName;
  readonly $fullTypeName: `${typeof PKG_V35}::config::Setting<${ToTypeStr<T0>}>`;
  ...
  private constructor(typeArgs: [ToTypeStr<T0>], fields: SettingFields<T0>) {
    this.$fullTypeName = composeSuiType(
      Setting.$typeName,
      ...typeArgs,
    ) as `${typeof PKG_V35}::config::Setting<${ToTypeStr<T0>}>`;
  ...
  static reified<T0 extends Reified<TypeArgument, any>>(
    ...
    return {
      ...
      fullTypeName: composeSuiType(
        Setting.$typeName,
        ...[extractType(T0)],
      ) as `${typeof PKG_V35}::config::Setting<${ToTypeStr<ToTypeArgument<T0>>}>`,
