...
export function isConfig(type: string): boolean {
  ...
  return type.startsWith(`${PKG_V35}::config::Config` + "<");
}
...
export class Config<T0 extends PhantomTypeArgument> implements StructClass {
  ...
  static readonly $typeName = `${PKG_V35}::config::Config`;
  static readonly $numTypeParams = 1;
  static readonly $isPhantom = [true] as const;

  readonly $typeName = Config.$typeName;
  readonly $fullTypeName: `${typeof PKG_V35}::config::Config<${PhantomToTypeStr<T0>}>`;
  ...
  private constructor(typeArgs: [PhantomToTypeStr<T0>], fields: ConfigFields<T0>) {
    this.$fullTypeName = composeSuiType(
      Config.$typeName,
      ...typeArgs,
    ) as `${typeof PKG_V35}::config::Config<${PhantomToTypeStr<T0>}>`;
  ...
  static reified<T0 extends PhantomReified<PhantomTypeArgument>>(
    ...
    return {
      ...
      fullTypeName: composeSuiType(
        Config.$typeName,
        ...[extractType(T0)],
      ) as `${typeof PKG_V35}::config::Config<${PhantomToTypeStr<ToPhantomTypeArgument<T0>>}>`,
