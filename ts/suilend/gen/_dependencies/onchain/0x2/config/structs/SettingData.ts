...
export function isSettingData(type: string): boolean {
  ...
  return type.startsWith(`${PKG_V35}::config::SettingData` + "<");
}
...
export class SettingData<T0 extends TypeArgument> implements StructClass {
  ...
  static readonly $typeName = `${PKG_V35}::config::SettingData`;
  static readonly $numTypeParams = 1;
  static readonly $isPhantom = [false] as const;

  readonly $typeName = SettingData.$typeName;
  readonly $fullTypeName: `${typeof PKG_V35}::config::SettingData<${ToTypeStr<T0>}>`;
  ...
  private constructor(typeArgs: [ToTypeStr<T0>], fields: SettingDataFields<T0>) {
    this.$fullTypeName = composeSuiType(
      ...
      ...typeArgs,
    ) as `${typeof PKG_V35}::config::SettingData<${ToTypeStr<T0>}>`;
  ...
  static reified<T0 extends Reified<TypeArgument, any>>(
    ...
    return {
      ...
      fullTypeName: composeSuiType(
        SettingData.$typeName,
        ...[extractType(T0)],
      ) as `${typeof PKG_V35}::config::SettingData<${ToTypeStr<ToTypeArgument<T0>>}>`,
