...
export function isExtension(type: string): boolean {
  ...
  return type === `${PKG_V35}::kiosk_extension::Extension`;
}
...
export class Extension implements StructClass {
  ...
  static readonly $typeName = `${PKG_V35}::kiosk_extension::Extension`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = Extension.$typeName;
  readonly $fullTypeName: `${typeof PKG_V35}::kiosk_extension::Extension`;
  readonly $typeArgs: [];
  ...
  private constructor(typeArgs: [], fields: ExtensionFields) {
    this.$fullTypeName = composeSuiType(
      Extension.$typeName,
      ...typeArgs,
    ) as `${typeof PKG_V35}::kiosk_extension::Extension`;
