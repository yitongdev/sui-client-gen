...
export function isDenyList(type: string): boolean {
  ...
  return type === `${PKG_V35}::deny_list::DenyList`;
}
...
export class DenyList implements StructClass {
  ...
  static readonly $typeName = `${PKG_V35}::deny_list::DenyList`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = DenyList.$typeName;
  readonly $fullTypeName: `${typeof PKG_V35}::deny_list::DenyList`;
  readonly $typeArgs: [];
  ...
  private constructor(typeArgs: [], fields: DenyListFields) {
    this.$fullTypeName = composeSuiType(
      DenyList.$typeName,
      ...typeArgs,
    ) as `${typeof PKG_V35}::deny_list::DenyList`;
