...
export function isConfigKey(type: string): boolean {
  ...
  return type === `${PKG_V35}::deny_list::ConfigKey`;
}
...
export class ConfigKey implements StructClass {
  ...
  static readonly $typeName = `${PKG_V35}::deny_list::ConfigKey`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = ConfigKey.$typeName;
  readonly $fullTypeName: `${typeof PKG_V35}::deny_list::ConfigKey`;
  readonly $typeArgs: [];
  ...
  private constructor(typeArgs: [], fields: ConfigKeyFields) {
    this.$fullTypeName = composeSuiType(
      ConfigKey.$typeName,
      ...typeArgs,
    ) as `${typeof PKG_V35}::deny_list::ConfigKey`;
