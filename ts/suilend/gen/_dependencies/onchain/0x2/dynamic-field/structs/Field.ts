...
export function isField(type: string): boolean {
  ...
  return type.startsWith(`${PKG_V35}::dynamic_field::Field` + "<");
}
...
> = Reified<Field<T0, T1>, FieldFields<T0, T1>>;
  ...
  implements StructClass
{
  ...
  static readonly $typeName = `${PKG_V35}::dynamic_field::Field`;
  static readonly $numTypeParams = 2;
  static readonly $isPhantom = [false, false] as const;

  readonly $typeName = Field.$typeName;
  readonly $fullTypeName: `${typeof PKG_V35}::dynamic_field::Field<${ToTypeStr<T0>}, ${ToTypeStr<T1>}>`;
  ...
  private constructor(
    ...
    fields: FieldFields<T0, T1>,
  ) {
    this.$fullTypeName = composeSuiType(
      Field.$typeName,
      ...typeArgs,
    ) as `${typeof PKG_V35}::dynamic_field::Field<${ToTypeStr<T0>}, ${ToTypeStr<T1>}>`;
  ...
  >(T0: T0, T1: T1): FieldReified<ToTypeArgument<T0>, ToTypeArgument<T1>> {
    return {
      typeName: Field.$typeName,
      fullTypeName: composeSuiType(
        Field.$typeName,
        ...[extractType(T0), extractType(T1)],
      ) as `${typeof PKG_V35}::dynamic_field::Field<${ToTypeStr<ToTypeArgument<T0>>}, ${ToTypeStr<ToTypeArgument<T1>>}>`,
      ...
      fromFields: (fields: Record<string, any>) =>
        Field.fromFields([T0, T1], fields),
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        Field.fromFieldsWithTypes([T0, T1], item),
      ...
      fromSuiParsedData: (content: SuiParsedData) =>
        Field.fromSuiParsedData([T0, T1], content),
      fromSuiObjectData: (content: SuiObjectData) =>
        Field.fromSuiObjectData([T0, T1], content),
      fetch: async (client: SuiClient, id: string) =>
        Field.fetch(client, [T0, T1], id),
  ...
  toJSONField() {
    ...
    return {
      id: this.id,
      name: fieldToJSON<T0>(typeArg0, this.name),
