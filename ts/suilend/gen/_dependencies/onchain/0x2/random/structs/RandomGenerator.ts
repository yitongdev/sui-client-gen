...
export function isRandomGenerator(type: string): boolean {
  ...
  return type === `${PKG_V35}::random::RandomGenerator`;
}
...
export class RandomGenerator implements StructClass {
  ...
  static readonly $typeName = `${PKG_V35}::random::RandomGenerator`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = RandomGenerator.$typeName;
  readonly $fullTypeName: `${typeof PKG_V35}::random::RandomGenerator`;
  readonly $typeArgs: [];
  ...
  private constructor(typeArgs: [], fields: RandomGeneratorFields) {
    this.$fullTypeName = composeSuiType(
      ...
      ...typeArgs,
    ) as `${typeof PKG_V35}::random::RandomGenerator`;
