...
export function isRandomInner(type: string): boolean {
  ...
  return type === `${PKG_V35}::random::RandomInner`;
}
...
export class RandomInner implements StructClass {
  ...
  static readonly $typeName = `${PKG_V35}::random::RandomInner`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = RandomInner.$typeName;
  readonly $fullTypeName: `${typeof PKG_V35}::random::RandomInner`;
  readonly $typeArgs: [];
  ...
  private constructor(typeArgs: [], fields: RandomInnerFields) {
    this.$fullTypeName = composeSuiType(
      ...
      ...typeArgs,
    ) as `${typeof PKG_V35}::random::RandomInner`;
