...
export function isVersioned(type: string): boolean {
  ...
  return type === `${PKG_V35}::versioned::Versioned`;
}
...
export class Versioned implements StructClass {
  ...
  static readonly $typeName = `${PKG_V35}::versioned::Versioned`;
  static readonly $numTypeParams = 0;
  static readonly $isPhantom = [] as const;

  readonly $typeName = Versioned.$typeName;
  readonly $fullTypeName: `${typeof PKG_V35}::versioned::Versioned`;
  readonly $typeArgs: [];
  ...
  private constructor(typeArgs: [], fields: VersionedFields) {
    this.$fullTypeName = composeSuiType(
      Versioned.$typeName,
      ...typeArgs,
    ) as `${typeof PKG_V35}::versioned::Versioned`;
